...
CT-V2-08 adds bounded deterministic source-first language perception and state formation. CT-V2-09 adds source-first Journal capture with default silence. CT-V2-10 adds pure deterministic Biographer coverage: open-story and targeted postures, structural gap maps, one-question semantic plans, user-controlled skip/defer/decline/private outcomes, and state-based anti-repetition. Only qualification composes these foundations with the synthetic store; no Android, model, retrieval, or production authority is granted.

CT-V2-11 adds purpose-bound, read-only longitudinal retrieval and immutable context packets. Eligibility precedes deterministic structural/lexical ranking; correction, privacy, lifecycle, unresolved identity, and counterevidence remain visible; explicit budgets bound object, excerpt, text, and dependency depth. Journal ordinary retrieval remains empty, Biographer target authority remains CT-V2-10, and Therapy route authority remains CT-V2-05. Only qualification adapts the CT-V2-07 reader; no production or model integration is granted.

CT-V2-12 adds a qualification-only Therapy longitudinal composition module. It admits the current user source, preserves CT-V2-04 safety and CT-V2-05 route authority, retrieves CT-V2-11 history only after the route is fixed and only at the pre-turn revision, and narrows planner-visible context to a bounded renderer-support envelope. Longitudinal memory cannot select safety, route, technique, mode, or evidence lifecycle; ordinary surfacing is at most one direct structural reference. No model, Android, production store, or new therapeutic authority is introduced.
