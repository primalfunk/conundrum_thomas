...
class CTV215R1ProductionUiInstrumentedTest {
    ...
    private fun send(text: String, expectedAction: String?, silent: Boolean = false): CoreOrdinaryTherapyState {
        ...
        println("R1 CLICK_TARGET_BEFORE " + compose.onNodeWithTag("commit-turn").fetchSemanticsNode().boundsInWindow)
        compose.onNodeWithTag("commit-turn").performClick()
        println("R1 CLICK_RETURN processing=" + model.state.value.processing + " draftLength=" + model.state.value.draft.length)
