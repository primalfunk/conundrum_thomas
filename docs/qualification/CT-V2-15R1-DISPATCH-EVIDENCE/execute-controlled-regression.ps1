...
try {
    ...
    $methods = @(
        ,@('f','com.conundrum.thomas.v2.CTV215R1DispatchInstrumentedTest#keyboardMovementCannotStrandAnIntendedSubmission')
    )
    foreach ($method in $methods) {
        if ($method[0] -ne 'f') { & $adb -s $serial shell am force-stop $fixture }
