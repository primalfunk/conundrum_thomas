...
class ThomasProductionRuntime(
    ...
    fun submit(request: ProductionTurnRequest): ProductionTurnResult {
        println("R1_DISPATCH t=" + System.nanoTime() + " RUNTIME_ENTER index=" + request.clientTurnIndex)
        ...
        println("R1_DISPATCH t=" + System.nanoTime() + " PIPELINE_ACCEPT index=" + request.clientTurnIndex)
        return try {
            when (request.mode) {
    ...
    private fun render(command: GovernedRenderCommand): GovernedRenderResult {
        println("R1_DISPATCH t=" + System.nanoTime() + " RENDER_ENTER act=" + command.semanticAct)
