...
class CTV215R1ProductionUiInstrumentedTest {
    ...
    private fun send(text: String, expectedAction: String?, silent: Boolean = false): CoreOrdinaryTherapyState {
        ...
        println("PRACTICAL_UI_PRE mode="+model.state.value.mode+" support="+model.state.value.therapySupport+" snapshot="+runtime.snapshot()+" session="+session()+" renderHistory="+read<RenderHistoryState>(runtime, "renderHistory"))
        ...
        println("PRACTICAL_UI_AFTER status="+model.state.value.status+" support="+model.state.value.therapySupport+" session="+session())
        val state = session()
    ...
    @Test fun cActualInputControlsAndSemanticProgression() {
        ...
        compose.onNodeWithText("Practical").assertIsSelected()
        println("PRACTICAL_UI_SELECTED="+model.state.value.therapySupport)
