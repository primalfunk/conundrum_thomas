...
try {
    ...
    $streamProcess = Start-Process -FilePath $adb -ArgumentList @('-s',$serial,'logcat',"--uid=$fixtureUid",'-v','threadtime') -WindowStyle Hidden -PassThru -RedirectStandardOutput (Join-Path $destination 'fixture-continuous.txt') -RedirectStandardError (Join-Path $destination 'fixture-continuous-errors.txt')
    Copy-Item thomas/runtime/src/main/kotlin/com/conundrum/thomas/v2/runtime/ThomasProductionRuntime.kt (Join-Path $destination 'diagnostic-runtime-source.kt')
    $methods = @(
        ...
        @('d','com.conundrum.thomas.v2.CTV215R1ProductionUiInstrumentedTest#dActualUiIdentityAfterProcessReopen'),
        @('e','com.conundrum.thomas.v2.CTV215R1ProductionUiInstrumentedTest#eSecondReopenDurabilityAndSubsequentIdentity')
    )
    ...
    if ($streamProcess -and -not $streamProcess.HasExited) { Stop-Process -Id $streamProcess.Id -ErrorAction SilentlyContinue }
