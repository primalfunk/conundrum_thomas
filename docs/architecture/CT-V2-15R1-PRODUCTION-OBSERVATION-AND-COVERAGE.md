...
Material revisions compare typed resolutions and normalized values, excluding submission IDs, evidence-reference IDs, and action-history bookkeeping. Case, repeated whitespace, and terminal punctuation do not themselves advance evidence. Internal punctuation is preserved, including decimal points; the conventional comma after an initial Yes/No is normalized for supported short replies. Unsupported paraphrases remain unclassified; their source text is captured, but they establish neither semantic equivalence nor novelty. A supported new detail or correction changes the typed state. Summary delivery and correction acknowledgement are explicit session events and may also change the next evaluated state.
...
The current fixture has three methods: aCompleteTypedProductionScenarios, bColdReopenDurabilityAndCoverage, and cActualInputControlsAndSemanticProgression. Run them in that order under the authorized isolated fixture/data-custody boundary, with fixture-only process stops before b and c. The third method drives actual Compose controls and requires admitted user input. Preserve the valued canonical installation and remove only disposable packages after recording custody comparisons. Do not run predecessor destructive instrumentation under the canonical package. Compilation is not device qualification; the latest report records the remaining failed UI gate.

## Qualified renderer recurrence repair and remaining UI admission boundary

The bounded recurrence repair leaves CT-V2-05 action selection and CT-V2-13 duplicate validation unchanged. GovernedLanguageRenderer.fallback now validates the full finite command-authorized reference repertoire before explicit exhaustion. TherapyRenderCommandAdapter retains the original further-expression invitation and provides two equivalent deterministic forms. No procedural state, route choice, safety evaluation or persistence writer was added to the renderer; linguistic history cannot become procedural evidence.

The repaired runtime passed the complete isolated A–N production and reopen scenarios on TCL 9491G. Actual UI admission after reopen remains blocked: ThomasViewModel initializes its turn counter from store revision, although question-only interactions can consume indices without a store write. That can reuse an existing source identity; the store correctly rejects the conflicting payload. A rendered response alone does not establish a committed user turn. MainActivity already marks such input Not saved.

The [recurrence repair qualification](../qualification/CT-V2-15R1-RECURRENCE-REPAIR-QUALIFICATION.md) records the strict renderer tests, physical passes, remaining UI failure and exact identity diagnostic. No turn-identity or persistence repair is included in this change, and R1 remains unsealed.
