...
class GovernedLanguageEvidencePipeline(
    private val admission: LongitudinalAdmissionController,
    private val reader: LongitudinalReader,
    private val classification: StoreDataClassification,
    ...
    fun process(sourceRevisionId: SourceRecordId, context: PerceptionContext = PerceptionContext()): GovernedLanguageProcessingResult {
        require(reader.classification == classification)
        val source = requireNotNull(reader.snapshot().sources.firstOrNull { it.id == sourceRevisionId }) {
    ...
    fun formState(): FormedLongitudinalState = stateFormer.form(stateEvidence(reader))
    ...
    private fun submitEvidence(source: SourceRecord, bundle: EvidenceBundle, result: LanguagePerceptionResult): AdmissionResult {
        ...
        val priorRevision = reader.redactedAdmissionHistory()
            .firstOrNull { it.idempotencyKey == idempotency && it.storeRevision != null }
            ?.storeRevision?.minus(1) ?: reader.currentStoreRevision()
    ...
    private fun admitNewContradictions(): List<AdmissionResult> {
        val evidence = stateEvidence(reader)
